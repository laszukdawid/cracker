...
class Configuration:
    ...
    language_file = "config/voices.json"
    DEFAULT_CONFIG_PATH = "config/default_setting.ini"
    ...
    def apply_config(self, configuration: ConfigParser) -> Dict[str, Any]:
        ...
        for speaker in configuration.sections():
            self._logger.debug(speaker)
            if speaker == "Cracker":
                continue
            _config[speaker.lower()] = {
                "voice": configuration[speaker]["voice"],
                "credentials_file": configuration[speaker].get("credentials_file", ""),
            }
